...
import { exportDeviceInventory } from '../../utils/electricity-violation-production/deviceInventoryExcelUtils';

const DeviceInventory = ({ readOnly, initialDevices }) => {
  const { deviceInventory, updateDeviceInventory } = useSharedData();
  const [editingKey, setEditingKey] = React.useState('');

  // Use initialDevices if provided (for read-only view), otherwise use deviceInventory from context
  const devices = initialDevices || deviceInventory;
  ...
  const handleDelete = (key) => {
    if (readOnly) return;
    updateDeviceInventory(devices.filter(item => item.key !== key));
  ...
  const handleAdd = () => {
    ...
    updateDeviceInventory([...devices, newData]);
