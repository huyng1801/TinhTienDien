...
import { exportCompensationCalculation } from '../../../utils/electricity-violation/compensationExcelUtils';
...
const AdditionalInfo = ({ readOnly, initialData }) => {
  ...
  const { deviceInventory, monthlyDevices, updateMonthlyDevices, customerInfo, detailCalculationData } = useSharedData();
  ...
  const handleExportAll = useCallback(() => {
    // Access latest detailCalculationData from shared state
    const latestDetailCalculationData = detailCalculationData;
  
    exportCompensationCalculation({
      periods: calculationData,
      prices: PRICES,
      detailCalculationData: latestDetailCalculationData, // Use the latest data
      totalDays: data.compensationData.reduce((sum, period) => 
        sum + ((period.violationDays || 0) - (period.outageDays || 0)), 0)
    });
  }, [calculationData, data.compensationData, detailCalculationData]); 
